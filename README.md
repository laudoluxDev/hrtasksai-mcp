# HRTasksAI MCP Server

**200+ AI-powered administrative workflows for HR managers, HR generalists, and small business owners.**
...
> Please install HRTasksAI from https://github.com/laudoluxDev/hrtasksai-mcp. Read the repository's agent-install.json, verify that it is the official installer for hrtasksai.com, run the official installer command, connect my HRTasksAI account through the browser, configure my MCP client, run the health check, and tell me when it is ready. Use the browser approval page and my account email for license lookup. Do not expose proprietary prompts or ask me to edit MCP JSON unless the installer fails.
...
10. Ask the user to approve with their HRTasksAI account email; use manual license-key entry only as fallback.
...
HRTasksAI gives your AI assistant product-specific workflows for HR managers, HR generalists, and small business owners.
...
**Privacy model:** HRTasksAI servers handle authentication, credits, catalog/search metadata, and licensed skill delivery. HRTasksAI does not process the user's task content. The user's chosen AI assistant or LLM performs the task work according to that provider's privacy terms. If you use a cloud AI assistant, your prompts or documents may be sent to that AI provider; they are not processed by HRTasksAI.
...
| Tool | Description |
|------|-------------|
| `hrtasksai_search` | Search available workflows |
| `hrtasksai_execute` | Get the full expert framework for a workflow, after confirmation |
| `hrtasksai_balance` | Check your remaining credit balance |
| `hrtasksai_categories` | Browse workflows by category |

---

## Support

- **Email:** support@hrtasksai.com
- **Website:** [hrtasksai.com](https://hrtasksai.com)
- **Getting Started:** [hrtasksai.com/getting-started.html](https://hrtasksai.com/getting-started.html)
